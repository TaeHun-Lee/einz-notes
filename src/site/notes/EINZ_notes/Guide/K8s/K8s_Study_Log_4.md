---
{"dg-publish":true,"permalink":"/einz-notes/guide/k8s/k8s-study-log-4/","title":"4: 설정 분리(ConfigMap, Secret)와, L7 라우팅(Ingress)"}
---


# 설정 분리(ConfigMap, Secret)와, L7 라우팅(Ingress)
...
코드는 유지한 채 환경만 바꾸기 위해 `process.env`를 사용하도록 코드를 수정한다.
...
ConfigMap(일반 설정)과 Secret(민감 정보)을 정의하고, Deployment에서 이를 환경 변수(env)로 끌어다 쓰도록 설정한다.
...
K8S 내부에 Nginx 인그레스 컨트롤러 파드를 시스템 네임스페이스(`ingress-nginx`)에 배포한다.
...
접속 시 텍스트만 뱉어내는 가벼운 이미지(`hashicorp/http-echo`)를 활용해 Apple과 Banana 앱을 배포한다.
...
`main-ingress.yaml`을 통해 컨트롤러가 참고할 길 안내 규칙을 K8S에 전달한다.
...
개별 서비스가 아닌, 트래픽을 통제하는 Ingress 컨트롤러 자체를 외부로 노출시켜 라우팅을 검증한다.
