---
{"dg-publish":true,"permalink":"/einz-notes/guide/k8s/k8s-study-log-8/","title":"8: 로컬 GitOps CI/CD 파이프라인 구축 총정리 (Minikube + Gitea + Jenkins + ArgoCD)"}
---
...
개발자가 코드를 푸시하기만 하면 전체 파이프라인이 자동으로 굴러가도록 하려면 웹훅(Webhook)을 설정해 주어야 한다.
